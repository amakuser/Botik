...
Проект работает как единое desktop-приложение и как headless runtime для server/systemd.
Текущий primary GUI/product path — новый Tauri desktop shell + app-service + route-based frontend.
Legacy pywebview path пока сохранён только как quarantined fallback на период до legacy retirement.
...
- Legacy Windows launcher через `src/botik/windows_entry.py` сохранён только как quarantined fallback.
...
## Legacy fallback dashboard workspaces (reference only)

Ниже — краткая справка по legacy pywebview shell, который пока оставлен только для rollback и fallback troubleshooting.
Это не текущий primary product path и не основная операторская поверхность.
...
- rollback остаётся возможным, потому что legacy launcher пока не удалён.

### Legacy fallback path (quarantined)

- source/dev fallback GUI: `python -m src.botik.gui.app`
- packaged fallback GUI: `botik.exe`
- legacy launcher helper: `run_windows_gui.bat`

Эта ветка запуска оставлена только для rollback и fallback troubleshooting.
Primary operator path остаётся новым Tauri shell.

### Headless runtime paths
...
Dashboard Shell использует внешние manifests как источник операционного состояния без обязательной пересборки desktop binary.
...
## Legacy fallback dashboard supervision (reference only)
...
- [docs/migration/legacy-quarantine.md](docs/migration/legacy-quarantine.md)
