...
def test_models_endpoint_returns_fixture_snapshot(tmp_path: Path):
    ...
    settings = Settings(
        session_token="models-token",
        models_read_fixture_db_path=fixture_db_path,
        models_read_manifest_path=manifest_path,
    )
    app = create_app(settings)
    with TestClient(app) as client:
        response = client.get("/models", headers={"x-botik-session-token": "models-token"})

    assert response.status_code == 200
    payload = response.json()
    assert payload["source_mode"] == "fixture"
    assert payload["summary"]["total_models"] == 3
    assert payload["summary"]["active_declared_count"] == 2
    assert payload["summary"]["latest_run_scope"] == "futures"
    assert payload["scopes"][0]["active_model"] == "spot-champion-v3"
    assert payload["scopes"][1]["active_model"] == "futures-paper-v2"
    assert payload["registry_entries"][0]["model_id"] == "futures-paper-v2"
    assert payload["recent_training_runs"][0]["run_id"] == "run-futures-1"
