...
| Semantic auto-region | `semantic.spec.ts` + `semantic.helpers.ts` | Discovery via `data-ui-*` | No | Contract / canonical state regressions |
...
### Add a semantic-region test (`semantic.spec.ts`)
- After tagging a new page or component with `data-ui-*`
- After introducing a new canonical enum (e.g. `MODEL_STATE`)
- The test should assert: roles present, canonical state mapped, recommended_check sane
...
**Correct approach:** run `pwsh ./scripts/update-visual-baselines.ps1`, review the diff visually, then commit.
...
health, spot, futures, analytics, models, jobs, runtime, telegram, settings

### Routes with layout check only (no pixel baseline)
logs, diagnostics, market, orderbook, backtest
...
- Routes without full-page baseline (5 routes: logs, diagnostics, market, orderbook, backtest)

## Removed in M1 cleanup (2026-04-26)

The following vision-anchored / research-grade specs and helpers were retired and live in external backup `C:/ai/aiBotik_legacy_backup_2026-04-26/`:

- `interaction.spec.ts`, `live-backend.spec.ts`, `region-guardrail.spec.ts`
- `vision_loop.helpers.ts`, `vision_diff.*`, `vision_discover.helpers.ts`, `vision_interpret.*`
- `semantic_gap.helpers.ts`, `semantic-gap.spec.ts`
- `auto_test_gen.ts`, `auto-test-gen.spec.ts`
- `tests/vision/agent_audit.spec.ts` (research-grade LLM-judge)

Their value was research-grade and they all skipped without `OLLAMA_VISION=1`. The active vision lane is `tests/vision/vision.spec.ts` (Claude API or heuristic).
